...
export class NormalizationService {
  ...
  async findMatchingProduct(
    ...
    rawText: string,
    storeId?: string
  ): Promise<{ productId: string; productName: string; categoryId: string; confidence: number } | null> {
    ...
    let bestMatch: { productId: string; productName: string; categoryId: string; confidence: number } | null = null;
    let highestSim = 0;
   ...
   */
  async rememberAlias(productId: string, rawText: string, storeId?: string): Promise<void> {
    const cleanedRaw = this.cleanText(rawText);
    if (!cleanedRaw || cleanedRaw.length < 2) return;
  ...
  async applyUserRules(
    ...
    for (const rule of rules) {
      ...
      let targetText = '';
      if (rule.matchField === 'RAW_DESCRIPTION') targetText = this.cleanText(rawText);
      else if (rule.matchField === 'STORE_NAME' && storeName) targetText = this.cleanText(storeName);
      else if (rule.matchField === 'PRODUCT_NAME' && productName) targetText = this.cleanText(productName);
