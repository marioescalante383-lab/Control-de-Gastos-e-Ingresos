...
export interface PurchaseItemStockInput {
  productId?: string;
  name: string;
...
export class InventoryService {
   ...
   */
  async processPurchaseItems(
    userId: string,
    items: PurchaseItemStockInput[],
    date: Date,
    transactionId: string
  ): Promise<void> {
    for (const item of items) {
      if (item.affectsInventory === false) continue;
      ...
      let inv = await prisma.inventoryItem.findUnique({
        where: { productId },
      });
  ...
  async recordMovement(
    ...
    productId: string,
    quantity: number,
    movementType: 'CONSUMPTION' | 'EXPIRED' | 'LOST' | 'GIFTED' | 'MANUAL_ADJUSTMENT',
    notes?: string
