...
export class DashboardController {
  async getSummary(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      // 3. Gastos por tienda
      const storeMap: Record<string, { name: string; total: number; purchases: number }> = {};
      for (const t of transactions) {
      ...
      const timelineMap: Record<string, { date: string; expenses: number; incomes: number }> = {};
      for (const t of transactions) {
        const day = t.date.toISOString().split('T')[0];
        if (!timelineMap[day]) timelineMap[day] = { date: day, expenses: 0, incomes: 0 };
